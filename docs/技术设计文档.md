...
type StructureOptions =
  ...
  | { type: 'logic-chart'; direction: 'left' | 'right' }
...
```
算法：主轴 + 交替分支
1. 主轴为水平/垂直线（extraShape）
2. 一级子节点按 date 或顺序沿主轴排列
3. alternate=true 时奇偶节点在主轴两侧
4. 子节点的子节点沿分支方向延伸
```
...
1. 根节点在鱼头
2. 主干线沿水平方向
3. 一级子节点以 ±branchAngle 附在主干
4. 二级及以下沿一级分支方向延伸
5. extraShape：主干线、骨干线
```
...
算法：行映射
1. DFS 遍历生成行列表
2. 每行：depth 决定缩进，columns 决定显示字段
3. 节点位置按行高 × 行号计算
4. 连线：缩进竖线 + 水平线
```
...
> 两份文档版本须保持一致（当前 **v1.5-draft**）。功能 ID 定义以《功能特性规格》为准，接口与组件以本文档为准。
